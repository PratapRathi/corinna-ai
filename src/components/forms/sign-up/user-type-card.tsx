...
const UserTypeCard = ({ register, setUserType, text, title, userType, value }: Props) => {
  return (
    <Label htmlFor={value}>
      <Card className={cn("w-full cursor-pointer", userType == value && "border-orange-400")}>
        <CardContent className="flex justify-between p-2">
          <div className="flex items-center gap-3">
            <Card className={cn("flex justify-center p-3", userType == value && "border-orange-400")}>
              <User size={30} className={cn(userType == value ? "text-orange-400" : "text-gray-400")} />
            </Card>
            <div className="">
